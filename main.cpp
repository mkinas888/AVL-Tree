...
int main() {
    ...
    float Tk[1000];   // tablica kluczy węzłów
    int i,x,i1,i2;

    srand(time(NULL));        // inicjujemy generator pseudolosowy


    for(i = 0; i < 1000; i++)   // Tablicę wypełniamy wartościami kluczy
        Tk[i] = i + 1;
    
    for(i = 0; i < 2000; i++)  // Mieszamy tablicę
    {
        i1 = rand() % 1000;       // Losujemy 2 indeksy
        i2 = rand() % 1000;
    ...
    for(i = 0; i < 1000; i++)                                               // Create AVL tree from array
    {
